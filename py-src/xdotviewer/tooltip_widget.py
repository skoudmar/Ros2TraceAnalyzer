...
import pathlib
import os.path
sys.path.append(os.path.join(pathlib.Path(__file__).parent.resolve().parent.resolve().parent.resolve().parent.resolve(), "xdot.py"))
import xdot.ui

from r2ta_interface import R2TAInterface
from ros_element import ElementReference, ChartRequest, ChartValue, ChartType, ElementType
    ...
    def on_hover(self, element, action, tooltip):
        ...
        if element.tooltip is not None and reference is not None:
            if self.prev_element != element:
                ...
                image_path = self.r2ta.render(ChartRequest(
                    node=reference.node,
                    value=ChartValue.default_for(reference.element_type, reference.node_type),
