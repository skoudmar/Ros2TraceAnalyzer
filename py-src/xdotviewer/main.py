...
# The xdot.py has not been updated yet (latest version is 1.6, while most distros provide 1.4)
# This expectes the user to have xdot.py repository next to the Ros2TraceAnalyzer repository
sys.path.append(
    os.path.join(
        pathlib.Path(__file__).parent.parent.parent.parent.resolve(), "xdot.py"
    )
)
import argparse
...
from ros_element import ElementReference, ElementType, NodeType
...
class MyDotWindow(xdot.ui.DotWindow):
    ...
    def on_url_clicked(self, widget, url: str, event):
        ...
        if reference is not None and reference.element_type is not None:
            window = ChartWindow(self.r2ta, element=reference)
